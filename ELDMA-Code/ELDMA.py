...
def prepare_data(seperate=False):
    ...
    miRNA_fea = np.loadtxt("miRNA similarity-PCA.txt",dtype=float,delimiter=",")
    drug_fea  = np.loadtxt("Drug similarity-PCA.txt",dtype=float,delimiter=",")
    ...
    for i in range(0, interaction.shape[0]):   
        for j in range(0, interaction.shape[1]): 
            ...
            if interaction[i, j] == 1:                     
                ...
                tmp_fea = (drug_fea_tmp,miRNA_fea_tmp)   
                ...
                test_fea= (drug_fea_tmp1,miRNA_fea_tmp1) 
